...
RcppExport SEXP loglik_AtCtEt_esp_c(SEXP b_a, SEXP b_c, SEXP b_e, SEXP pheno_m, SEXP pheno_d, SEXP B_a_m, SEXP B_a_d, SEXP B_c_m, SEXP B_c_d, SEXP B_e_m, SEXP B_e_d) {
	...
	for(int i = 0; i<(0.5*num_m); i++)
	{
		int start = 2*i;
		int end = start + 1;
		V_m = exp(as_scalar(b_e_m.row(start)*be))*diag+exp(as_scalar(b_c_m.row(start)*bc))*k_m+exp(as_scalar(b_a_m.row(start)*ba))*k_m;
	...
	for(int i = 0; i<(0.5*num_d); i++)
	{
		int start = 2*i;
		int end = start + 1;
		arma::mat V_d = exp(as_scalar(b_e_d.row(start)*be))*diag+exp(as_scalar(b_c_d.row(start)*bc))*k_d+exp(as_scalar(b_a_d.row(start)*ba))*k_d_a;
